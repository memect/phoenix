...
from code_executor import Table
import datetime
...
@pytest.mark.asyncio
async def test_eval_engine():
    evaluation_engine = EvaluationEngine.from_data_path('resources/GDDH')
    result = await evaluation_engine.evaluate_program(program, 'test', keys=['参会登记日期截止日期'])
    print(result)